AmCharts.AmSerialChart = AmCharts.Class({
    ...
    countColumns: function() {
        ...
        for (h = 0; h < b; h++) {
            ...
            var g = e.stackType;
            if ("100%" == g || "regular" == g)
                for (f = !1, l = 0; l < c; l++) d = this.graphs[l], d.tcc = 1, d.valueAxis == e && "column" == d.type && (!f && d.stackable && (a++, f = !0), (!d.stackable && d.clustered || d.newStack) && a++, d.columnIndex = a - 1, d.clustered || (d.columnIndex = 0));
            if ("none" == g || "3d" == g) {
                f = !1;
                    ...
                    if (s && (s = s.axes[e.id].graphs[b.id])) {
                        if (s.values) {
                            ...
                            if (!isNaN(A)) {
                                ...
                                var H = AmCharts.getDecimals(A),
                                    k = k + A,
                                    k = AmCharts.roundTo(k, Math.max(n, H));
                                m++;
                                n = k / m
                            }
                        }
    ...
    formatString: function(a, b, c) {
        ...
        if (-1 != a.indexOf("[[category]]")) {
            ...
            if (this.categoryAxis.parseDates) {
                var f = this.balloonDateFormat,
                    l = this.chartCursor;
                l && (f = l.categoryBalloonDateFormat); - 1 != a.indexOf("[[category]]") && (f = AmCharts.formatDate(e, f, this), -1 != f.indexOf("fff") && (f = AmCharts.formatMilliseconds(f,
                    e)), e = f)
            }
        ...
        a =
            AmCharts.formatValue(a, b.values, ["percents"], f); - 1 != a.indexOf("[[") && (a = AmCharts.formatDataContextValue(a, b.dataContext));
        ...
        if (AmCharts.ifArray(a)) {
            ...
            var x = this.firstDayOfWeek,
                ...
                a = AmCharts.resetDateToMin(new Date(a[a.length -
                    1].time + 1.05 * k), this.minPeriod, 1, x).getTime(),
                z;
            ...
            if (this.parseDates && !this.equalSpacing) {
                ...
                if (0 < this.gridCountR)
                    for (b = d; b <= l; b++) {
                        ...
                        n = m = p;
                        if (!isNaN(y))
                            for (k = 1; k < v; k += y) this.gridAlpha = this.minorGridAlpha, q = p + B * k, q = AmCharts.resetDateToMin(new Date(q), e, y, x).getTime(), q = new this.axisItemRenderer(this, (q - this.startTime) * this.stepWidth), this.pushAxisItem(q);
                        this.gridAlpha = a
                    }
            } else if (!this.parseDates) {
                if (this.cellWidth = this.getStepWidth(b), b < l && (l = b), f += this.start, this.stepWidth = this.getStepWidth(b), 0 < l)
                    for (u = Math.floor(b / l), y = this.chooseMinorFrequency(u),
                        ...
                        if (q && !isNaN(y))
                            if (b / y == Math.round(b / y) || n) b / u == Math.round(b / u) || n || (this.gridAlpha = this.minorGridAlpha, m = void 0);
                            else continue;
                ...
                r = k;
                if (this.data[k].time != AmCharts.resetDateToMin(new Date(this.data[k].time), e, v, x).getTime())
                    for (x = 0, z = m, b = k; b < w; b++) p = this.data[b].time, this.checkPeriodChange(e, v, p, z) && (x++, 2 <= x && (r = b, b = w), z = p);
                q && 1 < v && (y = this.chooseMinorFrequency(v), AmCharts.getPeriodDuration(e, y));
                if (0 < this.gridCountR)
                    for (b = k; b < w; b++)
                        if (p = this.data[b].time, this.checkPeriodChange(e,
                                v, p, m) && b >= r) {
            ...
            for (b = 0; b < this.data.length; b++)
                if (h = this.data[b]) g = this.parseDates && !this.equalSpacing ? Math.round((h.time - this.startTime) * this.stepWidth + this.cellWidth / 2) :
